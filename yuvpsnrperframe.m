function PSNR = yuvpsnrperframe(File1,File2,width,height,format,parameter)
    ...
    if mod(framenumber1,1) ~= 0 | mod(framenumber2,1) ~= 0 | framenumber1~=framenumber2
        ...
        for cntf = 1:framenumber1
            ...
            if parameter == 'y'
                tmp=sum(sum((double(YUV1(:,:,1))-double(YUV2(:,:,1))).^2))/(width*height);
                if tmp ~= 0
                     PSNR(cntf) = 10*log10((255^2)/tmp)
                else
                     PSNR(cntf) = Inf;
                end
            elseif parameter == 'u'
                tmp=sum(sum((double(YUV1(:,:,2))-double(YUV2(:,:,2))).^2))/(width*height);
                if tmp ~= 0
                     PSNR(cntf) = 10*log10((255^2)/tmp)
                else
                     PSNR(cntf) = Inf;
            ...
            elseif parameter == 'v'
                tmp=sum(sum((double(YUV1(:,:,3))-double(YUV2(:,:,3))).^2))/(width*height);
                if tmp ~= 0
                     PSNR(cntf) = 10*log10((255^2)/tmp)
                else
                     PSNR(cntf) = Inf;
            ...
            elseif parameter == 'yuv'
                tmp=sum((double(YUV1(:))-double(YUV2(:))).^2)/length(YUV1(:));
                if tmp ~= 0
                     PSNR(cntf) = 10*log10((255^2)/tmp)
                else
                     PSNR(cntf) = Inf;
